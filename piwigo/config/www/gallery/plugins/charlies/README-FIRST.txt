...
If you get Charlie's as zip file (and not as automatically inserted plugin), you have just to unzip it then send is by ftp to piwigo/plugins the charlies folder to get piwigo/plugin/charlies.

At lest from 2.8.6 (but may be before), the piwigo include/category_default.inc.php file have a line:

$conf['upload_form_all_types'] = false;

that prevent loading of non image files. I don't use native piwigo uploading way usually, but ftp, so for me this is not a problem, but if you want to use native piwigo way, you may have to change this line to set "true":

$conf['upload_form_all_types'] = true;

Best done in local file editor plugin, now default.
